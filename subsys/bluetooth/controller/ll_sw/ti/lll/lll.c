...
static void swi_lll_cc13xx_cc26xx_isr(void *arg)
{
	DEBUG_RADIO_ISR(1);

	BT_DBG("IRQ %u", HAL_SWI_RADIO_IRQ);
...
#if defined(CONFIG_BT_CTLR_LOW_LAT) ||                                         \
	(CONFIG_BT_CTLR_ULL_HIGH_PRIO != CONFIG_BT_CTLR_ULL_LOW_PRIO)
static void swi_ull_cc13xx_cc26xx_isr(void *arg)
{
	DEBUG_TICKER_JOB(1);

	BT_DBG("IRQ %u", HAL_SWI_JOB_IRQ);
...
int lll_init(void)
{
	...
	IRQ_CONNECT(HAL_SWI_RADIO_IRQ, CONFIG_BT_CTLR_LLL_PRIO,
		    swi_lll_cc13xx_cc26xx_isr, NULL, 0);
#if defined(CONFIG_BT_CTLR_LOW_LAT) ||                                         \
	(CONFIG_BT_CTLR_ULL_HIGH_PRIO != CONFIG_BT_CTLR_ULL_LOW_PRIO)
...
int lll_prepare_done(void *param)
{
	...
	/* Ticker Job Silence */
	ret = ticker_job_idle_get(TICKER_INSTANCE_ID_CTLR, TICKER_USER_ID_LLL,
		   ...
		   struct lll_prepare_param *prepare_param, u8_t is_resume)
{
	...
	if (event.curr.abort_cb || (p && is_resume)) {
		...
		ret = ticker_start(TICKER_INSTANCE_ID_CTLR, TICKER_USER_ID_LLL,
				   TICKER_ID_LLL_PREEMPT, preempt_anchor,
				   preempt_to, TICKER_NULL_PERIOD,
				   TICKER_NULL_REMAINDER, TICKER_NULL_LAZY,
				   TICKER_NULL_SLOT, preempt_ticker_cb, NULL,
...
static void ticker_op_job_disable(u32_t status, void *op_context)
{
	...
	if (1 /* _radio.state != STATE_NONE */) {
		mayfly_enable(TICKER_USER_ID_ULL_LOW, TICKER_USER_ID_ULL_LOW,
			      0);
	}
}
#endif
...
{
	BT_DBG("%s:%d: assertion failed", file, line);
	while (1)
		;
}
#endif
